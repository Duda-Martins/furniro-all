import type { ProductModel } from "../models/ProductModel";
import { Badge } from "./Badge";
import { toast } from "react-toastify";
import { Link } from "react-router-dom";
import { useCartStore } from "../store/cartStore";

type ProductCardProps = {
    product: ProductModel;
};

const now = Date.now();

export function ProductCard({ product }: ProductCardProps) {
    const addItem = useCartStore((state) => state.addItem);

    const postedAt = new Date(product.postedAt ?? now);
    const diff = now - postedAt.getTime();
    const isNewThreshold = 2592000000; // 30 dias em milissegundos

    const safeDiscount = product.discount ?? 0;

    const originalPrice = product.price;

    const finalPrice =
        safeDiscount > 0
            ? Math.round(originalPrice * (1 - safeDiscount / 100))
            : originalPrice;

    function handleAddToCart() {
        addItem(product, null, 1);
        toast.success(`${product.name} added to cart!`);
    }

    return (
        <div className="group relative flex w-full flex-col items-start bg-[#F4F5F7]">
            <Link to={`/product/${product.id}`} className="w-full">
                <div className="relative">
                    <img
                        src={`/img/products/${product.image}`}
                        alt={product.name}
                        className="w-full h-75 object-cover"
                    />
                    {safeDiscount > 0 ? (
                        <Badge discount={safeDiscount} />
                    ) : diff <= isNewThreshold ? (
                        <Badge isNew />
                    ) : null}
                </div>
                <h3 className="font-poppins font-semibold text-[#3a3a3a] text-2xl/[120%] mt-4 mx-4">
                    {product.name}
                </h3>
                <p className="font-poppings text-[#898989] font-medium text-base/[150%] mx-4">
                    {product.description}
                </p>
                <div className="flex flex-row flex-wrap items-center gap-x-3 gap-y-1 mt-2 mx-4 mb-8">
                    <p className="font-poppins text-xl/[150%] text-[#3A3A3A] font-semibold whitespace-nowrap">
                        Rp {finalPrice.toLocaleString("id-ID")}
                    </p>

                    {safeDiscount > 0 && (
                        <p className="font-poppins text-[#B0B0B0] text-base/[150%] font-normal line-through whitespace-nowrap">
                            Rp {originalPrice.toLocaleString("id-ID")}
                        </p>
                    )}
                </div>
            </Link>

            <div className="absolute inset-0 bg-[#3A3A3A]/70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-center items-center z-20">
                <Link
                    to={`/product/${product.id}`}
                    className="absolute inset-0 z-10"
                />

                <button
                    onClick={handleAddToCart}
                    className="relative z-20 bg-white cursor-pointer text-[#B88E2F] font-semibold px-12 py-3 hover:bg-gray-100"
                >
                    Add to Cart
                </button>
                <div className="relative z-20 flex flex-row gap-4 mt-4">
                    <a
                        href="#"
                        className="flex flex-row justify-center items-center gap-1"
                    >
                        <svg
                            width="16"
                            height="16"
                            viewBox="0 0 16 16"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path
                                d="M12 10.6667C11.4747 10.6667 11 10.8734 10.644 11.2047L5.94 8.46671C5.97333 8.31337 6 8.16004 6 8.00004C6 7.84004 5.97333 7.68671 5.94 7.53337L10.64 4.79337C11 5.12671 11.4733 5.33337 12 5.33337C13.1067 5.33337 14 4.44004 14 3.33337C14 2.22671 13.1067 1.33337 12 1.33337C10.8933 1.33337 10 2.22671 10 3.33337C10 3.49337 10.0267 3.64671 10.06 3.80004L5.36 6.54004C5 6.20671 4.52667 6.00004 4 6.00004C2.89333 6.00004 2 6.89337 2 8.00004C2 9.10671 2.89333 10 4 10C4.52667 10 5 9.79337 5.36 9.46004L10.0587 12.2054C10.0211 12.3563 10.0014 12.5112 10 12.6667C10 13.0623 10.1173 13.4489 10.3371 13.7778C10.5568 14.1067 10.8692 14.3631 11.2346 14.5145C11.6001 14.6658 12.0022 14.7054 12.3902 14.6283C12.7781 14.5511 13.1345 14.3606 13.4142 14.0809C13.6939 13.8012 13.8844 13.4448 13.9616 13.0569C14.0387 12.6689 13.9991 12.2668 13.8478 11.9013C13.6964 11.5359 13.44 11.2235 13.1111 11.0038C12.7822 10.784 12.3956 10.6667 12 10.6667Z"
                                fill="white"
                            />
                        </svg>
                        <p className="font-poppins text-white font-semibold text-base/[150%]">
                            Share
                        </p>
                    </a>
                    <a
                        href="#"
                        className="flex flex-row justify-center items-center gap-1"
                    >
                        <svg
                            width="16"
                            height="16"
                            viewBox="0 0 16 16"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path
                                d="M10.08 7L11.08 8L14.52 4.55L11 1L10 2L11.8 3.8H2.00004V5.2H11.82L10.08 7ZM5.86004 9L4.86004 8L1.42004 11.5L4.91004 15L5.91004 14L4.10004 12.2H14V10.8H4.10004L5.86004 9Z"
                                fill="white"
                            />
                        </svg>
                        <p className="font-poppins text-white font-semibold text-base/[150%]">
                            Compare
                        </p>
                    </a>
                    <a
                        href="#"
                        className="flex flex-row justify-center items-center gap-1"
                    >
                        <svg
                            width="16"
                            height="16"
                            viewBox="0 0 16 16"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path
                                d="M7.99973 14.0361C-5.33333 6.66669 3.99999 -1.33331 7.99973 3.72539C12 -1.33331 21.3333 6.66669 7.99973 14.0361Z"
                                stroke="white"
                                strokeWidth="1.8"
                            />
                        </svg>
                        <p className="font-poppins text-white font-semibold text-base/[150%]">
                            Like
                        </p>
                    </a>
                </div>
            </div>
        </div>
    );
}
